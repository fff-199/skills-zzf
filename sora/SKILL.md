...
- If the user has a **video id** and wants a change â†?**remix**
- If the user has a **video id** and wants status or assets â†?**status/poll/download**
- If the user needs many prompts/assets â†?**create-batch**
- If the user asks for two versions with a small change (same shot, different subject/detail) â†?**create** the base, then **remix** for the variation
- Otherwise â†?**create** (or **create-and-poll** if they need a ready asset in one step)
