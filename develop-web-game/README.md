...
- Run bundled scripts relative to this folder, for example `./develop-web-game/scripts/...` from the repo root.

## Adapted Instructions

# Develop Web Game

Build games in small steps and validate every change. Treat each iteration as: implement 鈫?act 鈫?pause 鈫?observe 鈫?adjust.

## Skill paths (set once)

```bash
export SKILLS_ROOT="${SKILLS_ROOT:-$HOME/.agent CLI}"
export WEB_GAME_CLIENT="./develop-web-game/scripts/web_game_playwright_client.js"
export WEB_GAME_ACTIONS="./develop-web-game/references/action_payloads.json"
```
...
11. **Verify controls and state (multi-step focus).** Exhaustively exercise all important interactions. For each, think through the full multi-step sequence it implies (cause 鈫?intermediate states 鈫?outcome) and verify the entire chain works end-to-end. Confirm `render_game_to_text` reflects the same state shown on screen. If anything is off, fix and rerun.
...
14. **Iterate with small deltas.** Change one variable at a time (frames, inputs, timing, positions), then repeat steps 7鈥?3 until stable.
...
Test any new features added for the request and any areas your logic changes could affect. Identify issues, fix them, and re-run the tests to confirm they鈥檙e resolved.
...
- `$WEB_GAME_CLIENT` (installed default: `./develop-web-game/scripts/web_game_playwright_client.js`) 鈥?Playwright-based action loop with virtual-time stepping, screenshot capture, and console error buffering. You must pass an action burst via `--actions-file`, `--actions-json`, or `--click`.

## References

- `$WEB_GAME_ACTIONS` (installed default: `./develop-web-game/references/action_payloads.json`) 鈥?example action payloads (keyboard + mouse, per-frame capture). Use these to build your burst.
