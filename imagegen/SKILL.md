...
- If the user provides an input image (or says â€œedit/retouch/inpaint/mask/translate/localize/change only Xâ€? â†?**edit**
- Else if the user needs many different prompts/assets â†?**generate-batch**
- Else â†?**generate**
...
- If the result isnâ€™t clearly relevant or doesnâ€™t satisfy constraints, iterate with small targeted prompt changes; only ask a question if a missing detail blocks success.
...
- ui-mockup â€?app/web interface mockups that look shippable.
...
- precise-object-edit â€?remove/replace a specific element (incl. interior swaps).
...
- If the user says â€œa hero image for a landing pageâ€? you may add *layout/composition constraints* that are implied by that use (e.g., â€œgenerous negative space on the right for headline textâ€?.
- Do not introduce new creative elements the user didnâ€™t ask for (e.g., adding a mascot, changing the subject, inventing brand names/logos).
...
- If results feel â€œtackyâ€? add a brief â€œAvoid:â€?line (stock-photo vibe; cheesy lens flare; oversaturated neon; harsh bloom; oversharpening; clutter) and specify restraint (â€œeditorialâ€? â€œpremiumâ€? â€œsubtleâ€?.
