...
- Run bundled scripts relative to this folder, for example `./netlify-deploy/scripts/...` from the repo root.
...
- 鉁?Authenticated: Shows logged-in user email and site link status
- 鉂?Not authenticated: "Not logged into any site" or authentication error
...
**If already linked** 鈫?Skip to step 4
...
**"Not logged in"**
鈫?Run `npx netlify login`

**"No site linked"**
鈫?Run `npx netlify link` or `npx netlify init`

**"Build failed"**
鈫?Check build command and publish directory in netlify.toml or CLI prompts
鈫?Verify dependencies are installed
鈫?Review build logs for specific errors

**"Publish directory not found"**
鈫?Verify build command ran successfully
鈫?Check publish directory path is correct
...
```
The deploy needs escalated network access to deploy to Netlify. I can rerun the command with escalated permissions鈥攚ant me to proceed?
```
...
2. Set in Netlify dashboard: Site Settings 鈫?Environment Variables
