...
- ‚ú?Authenticated: Shows logged-in user email and site link status
- ‚ù?Not authenticated: "Not logged into any site" or authentication error
...
**If already linked** ‚Ü?Skip to step 4
...
**"Not logged in"**
‚Ü?Run `npx netlify login`

**"No site linked"**
‚Ü?Run `npx netlify link` or `npx netlify init`

**"Build failed"**
‚Ü?Check build command and publish directory in netlify.toml or CLI prompts
‚Ü?Verify dependencies are installed
‚Ü?Review build logs for specific errors

**"Publish directory not found"**
‚Ü?Verify build command ran successfully
‚Ü?Check publish directory path is correct
...
```
The deploy needs escalated network access to deploy to Netlify. I can rerun the command with escalated permissions‚Äîwant me to proceed?
```
...
2. Set in Netlify dashboard: Site Settings ‚Ü?Environment Variables
