...
- Run bundled scripts relative to this folder, for example `./render-deploy/scripts/...` from the repo root.
...
3) Add the MCP server with the the agent CLI:
```bash
agent CLI mcp add render --url https://mcp.render.com/mcp --bearer-token-env-var RENDER_API_KEY
```

4) Restart the agent, then retry `list_services()`.
