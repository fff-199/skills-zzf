...
- Run bundled scripts relative to this folder, for example `./notion-meeting-intelligence/scripts/...` from the repo root.

## Adapted Instructions

# Meeting Intelligence

Prep meetings by pulling Notion context, tailoring agendas/pre-reads, and enriching with the agent research.
...
5) Enrich with the agent research (industry insights, benchmarks, risks) and update the page with `Notion:notion-update-page` as plans change.
...
### 2) Choose format
- Status/update 鈫?status template.
- Decision/approval 鈫?decision template.
- Planning (sprint/project) 鈫?planning template.
- Retro/feedback 鈫?retrospective template.
- 1:1 鈫?one-on-one template.
- Ideation 鈫?brainstorming template.
...
- Add concise the agent research where helpful: market/industry facts, benchmarks, risks, best practices.
...
- `reference/` 鈥?template picker and meeting templates (e.g., `template-selection-guide.md`, `status-update-template.md`, `decision-meeting-template.md`, `sprint-planning-template.md`, `one-on-one-template.md`, `retrospective-template.md`, `brainstorming-template.md`).
- `examples/` 鈥?end-to-end meeting preps (e.g., `executive-review.md`, `project-decision.md`, `sprint-planning.md`, `customer-meeting.md`).
